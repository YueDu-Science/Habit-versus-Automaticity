...
% 0.125
habit = habitual - other_error;
for i = 1:size(habit,3)
   a = nanmean(habit(1,1:tmin(s),i)) - 0;
   habit(1,:,i) = habit(1,:,i) - a;
end
...
habit_perm = reshape(habit, 1200, numel(habit)/1200);
chance = repmat(0.125, 1200, size(habit_perm,2));
baseline = repmat(0, 1200, size(habit_perm,2));
other = reshape(other_error, 1200, numel(other_error)/1200);
clear temp;
x = 1:1200;
[C_global] = permutation_test_2tailed(habit_perm(x,:),baseline(x,:),1,1);
% find x where significant results were found
ind = find(nanmean((habit_perm(x,:)-baseline(x,:))') > C_global(1,:) | nanmean((habit_perm(x,:)-baseline(x,:))') < C_global(2,:));
% since we use a sliding window of 100ms
% we extend the ind by -50 and +50
IND_swap = ind;
IND_swap = [435-50,1150+50]; % this is besed on the orignal data without adjusted with tmin;
...
shadedErrorBar(xplot,nanmean(habit,3),seNaN(habit),{'-','color',zhongguo_hong})
f3 = plot(xplot,nanmean(habit,3),'-','color',zhongguo_hong,'Markersize',mks,'linewidth',lw);
...
plot([0 1.2], [0 0],'k--','linewidth',0.5);
