...
% This code only save data for Exp 1.
% it generates the file named: "Data_for_Analysis.mat"
...
%% Learning sessions; responding as slowly as they need          
        ...
        % RT when they made correct errors to revised stimuli when learning
            ...
            cr_habit_remap_trials = cr_habit_remap(cr_habit_remap.actual_choice == cr_habit_remap.proposed_choice,:);
...
%% Phit for habit test: forced-RT        
       ...
       % creat index for element whether mapped or remapped
        if ~isempty(data_tr_lett_prob_remap)
            ...
            habit_remap.correct(habit_remap.actual_choice == habit_remap.remapped_from) = 1;
        ...
        % this data file is for modeling
       ...
       [para, ycdf] = fit_unchanged(unchange_remap.real_prep_time, unchange_remap.correct);
        ...
        elseif ind > 300
            interval1 = find(habit_remap.real_prep_time >= (ind - 300)/1000 & habit_remap.real_prep_time <= ind/1000);
        end
        interval2 = find(habit_remap.real_prep_time >= (ind+1)/1000 & habit_remap.real_prep_time <= (ind+300)/1000);
        interval3 = find(habit_remap.real_prep_time >= (ind+301)/1000 & habit_remap.real_prep_time <= (ind+600)/1000);

        d = length(habit_remap.correct);
        habit_index_1 = (sum(habit_remap.correct(interval1))-sum(other_remap.correct(interval1))/6)/length(habit_remap.correct(interval1));
        habit_index_2 = (sum(habit_remap.correct(interval2))-sum(other_remap.correct(interval2))/6)/length(habit_remap.correct(interval2));
        habit_index_3 = (sum(habit_remap.correct(interval3))-sum(other_remap.correct(interval3))/6)/length(habit_remap.correct(interval3));

        habit_index = [habit_index_1 habit_index_2 habit_index_3];
        Data_online(g).habit_index{s,1} = habit_index; % save data for plot group mean later
        clear habit_index;
...
DATA_NA.Online = Data_online;%% create a table file containing data that can be used for analysis and plot
